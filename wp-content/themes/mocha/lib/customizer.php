 ...
 */
function mocha_customize_register( $wp_customize ){

	$priority = 200;
	$sections = array();
	$mocha_options = mocha_options();	
	$i = 0;
	foreach ( $mocha_options->sections as $section) {
		...
		if ( isset($section['fields']) && is_array($section['fields']) ) {
			...
			foreach ($section['fields'] as $field ){
				...
				if ($mocha_options->get($customize)) {
					...
					$field['settings'] = $mocha_options->args['opt_name'].'['.$field['id'].']';
	...
	foreach ( $sections as $section ) {
		...
		foreach ($section['fields'] as $field ){ 
			...
			$wp_customize->add_setting( 
					$mocha_options->args['opt_name'].'['.$field['id'].']', 
					array(
						'default'  	=> $mocha_options->get($field['id']),
    					'type' 		=> 'option',
...
add_action( 'customize_register', 'mocha_customize_register' );
