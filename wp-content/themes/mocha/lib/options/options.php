...
if ( ! class_exists('Mocha_Options') ){
	...
	class Mocha_Options{
		...
		public $dir = MOCHA_DIR;
		public $url = MOCHA_URL;
		public $page = '';
		public $args = array();
		...
		public function __construct($sections = array(), $args = array()){
			...
			$defaults['page_title'] = esc_html__('Options', 'mocha');
			...
			$this->args = apply_filters( 'mocha_options_args_' . $this->args['opt_name'], $this->args );
			...
			//get sections
			$this->sections = apply_filters( 'mocha_options_sections_' . $this->args['opt_name'], $sections );				
			...
			add_action('mocha-opts-page-before-form-'.$this->args['opt_name'], array(&$this, '_errors_js'), 1);
				
			//add the js for the warning handling before the form
			add_action('mocha-opts-page-before-form-'.$this->args['opt_name'], array(&$this, '_warnings_js'), 2);
		...
		public function cleanCookie() {
			if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('mocha-opts-saved') == '1' && is_array($_COOKIE) ){
		...
		function _enqueue(){
			...
			wp_enqueue_style(
				'mocha-opts-css',
				MOCHA_URL . '/admin/css/options.css',
				array('farbtastic'),
				time(),
				'all'
			);
			...
			wp_enqueue_script(
				'mocha-opts-js',
				MOCHA_URL.'/admin/js/options.js',
				array('jquery'),
				time(),
				true
			);
			
			wp_localize_script('mocha-opts-js', 'mocha_opts', array('reset_confirm' => esc_html__('Are you sure? Resetting will loose all custom values.', 'mocha'), 'opt_name' => $this->args['opt_name']));
		...
		function _download_options(){
			...
			$backup_options['mocha-opts-backup'] = '1';
		...
		function _load_page(){
			...
			do_action('mocha-opts-load-page-'.$this->args['opt_name'], $screen);
				
		}//function


		/**
		 * do action mocha-opts-admin-head for theme options page
		 *
		 * @since Mocha_Options 1.0
		 */
		function admin_head(){
				
			do_action('mocha-opts-admin-head-'.$this->args['opt_name'], $this);
		...
		function _register_setting(){
			...
			do_action('mocha-opts-register-settings-'.$this->args['opt_name']);
		...
		function _validate_options($plugin_options){
			...
			set_transient('mocha-opts-saved', '1', 1000 );
			...
			if(!empty($plugin_options['import'])){
				...
				if(is_array($imported_options) && isset($imported_options['mocha-opts-backup']) && $imported_options['mocha-opts-backup'] == '1'){
			...
			if($this->errors){
				set_transient('mocha-opts-errors-'.$this->args['opt_name'], $this->errors, 1000 );
			}//if errors
				
			if($this->warnings){
				set_transient('mocha-opts-warnings-'.$this->args['opt_name'], $this->warnings, 1000 );
			}//if errors
				
			do_action('mocha-opts-options-validate-'.$this->args['opt_name'], $plugin_options, $this->options);
				...
				if(isset($section['fields'])){
					...
					foreach($section['fields'] as $fieldk => $field){
						...
						if(isset($field['validate'])){
							$validate = 'Mocha_Validation_'.$field['validate'];
		...
		public function _options_form(){
			...
			$this->options['last_tab'] = (isset($_GET['tab']) && !get_transient('mocha-opts-saved'))?$_GET['tab']:$this->options['last_tab'];
			...
			<div class="cpannel-button">
				<button id="cpanel-submit" class="btn btn-inverse" type="submit"><?php esc_html_e( 'SAVE', 'mocha' ); ?></button>
				<button id="cpanel-reset" class="btn btn-inverse" type="button"><?php esc_html_e( 'RESET', 'mocha' ); ?></button>
			</div>
			<?php
			echo '</div>';
		...
		public function getFieldInstance( $field = array() ){
			...
			if ( !class_exists($classname) ){
				$classfile = MOCHA_DIR."/options/fields/$type/field_$type.php";
		...
		public function getCpanelField( $field ){
			...
			$classname = 'Mocha_Options_'.$field['type'];
			if ( !class_exists($classname) ){
				$classfile = MOCHA_DIR.'/options/fields/'.$field['type'].'/field_'.$field['type'].'.php';
				if ( file_exists($classfile) ){
					include $classfile;
				}
			}
		...
		function _options_page_html(){
			...
			echo '<h2 id="mocha-opts-heading">'.get_admin_page_title().'</h2>';
			
			echo isset($this->args['intro_text']) ? $this->args['intro_text'] : '';
		
			do_action('mocha-opts-page-before-form-'.$this->args['opt_name']);
		
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="mocha-options-form">';
			...
			echo '<div id="mocha-opts-header">';
			submit_button('', 'primary', '', false);
			submit_button(esc_html__('Reset to Defaults', 'mocha'), 'secondary', $this->args['opt_name'].'[defaults]', false);
			echo '<div class="clear"></div><!--clearfix-->';
			echo '</div>';
		
			if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('mocha-opts-saved') == '1'){
				if(isset($this->options['imported']) && $this->options['imported'] == 1){
					echo '<div id="mocha-opts-imported">'.apply_filters('mocha-opts-imported-text-'.$this->args['opt_name'], wp_kses( __( '<strong>Settings Imported!</strong>', 'mocha'), array( 'strong' => array() ) ) ).'</div>';
				}else{
					echo '<div id="mocha-opts-save">'.apply_filters('mocha-opts-saved-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings Saved!</strong>', 'mocha'), array( 'strong' => array() ) ) ).'</div>';
					
				}
				delete_transient('mocha-opts-saved');
			}
			echo '<div id="mocha-opts-save-warn">'.apply_filters('mocha-opts-changed-text-'.$this->args['opt_name'], wp_kses( __('<strong>Settings have changed!, you should save them!</strong>', 'mocha'), array( 'strong' => array() ) ) ).'</div>';
			echo '<div id="mocha-opts-field-errors">'. wp_kses(__('<strong><span></span> error(s) were found!</strong>', 'mocha'), array( 'strong' => array(), 'span' => array() ) ).'</div>';
		
			echo '<div id="mocha-opts-field-warnings">'.esc_html__('<strong><span></span> warning(s) were found!</strong>', 'mocha').'</div>';
		
			echo '<div class="clear"></div><!--clearfix-->';
		
			echo '<div id="mocha-opts-sidebar">';
			echo '<ul id="mocha-opts-group-menu">';
			foreach($this->sections as $k => $section){
				$icon = (!isset($section['icon']))?'<img src="'.$this->url.'img/glyphicons/glyphicons_019_cogwheel.png" alt=""/> ':'<img src="'.$section['icon'].'" alt=""/> ';
				echo '<li id="'.$k.'_section_group_li" class="mocha-opts-group-tab-link-li">';
				echo '<a href="javascript:void(0);" id="'.$k.'_section_group_li_a" class="mocha-opts-group-tab-link-a" data-rel="'.$k.'">'.$icon.'<span>'.$section['title'].'</span></a>';
				echo '</li>';
			}
				
			do_action('mocha-opts-after-section-menu-items-'.$this->args['opt_name'], $this);
			
			echo '<li id="import_export_default_section_group_li" class="mocha-opts-group-tab-link-li">';
			echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="mocha-opts-group-tab-link-a" data-rel="import_export_default"><img src="'.$this->url.'/options/img/glyphicons/glyphicons_082_roundabout.png" /> <span>'.esc_html__('Import / Export', 'mocha').'</span></a>';
			echo '</li>';
		
		
			if(true === $this->args['dev_mode']){
				echo '<li id="dev_mode_default_section_group_li" class="mocha-opts-group-tab-link-li">';
				echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="mocha-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> <span>'.esc_html__('Dev Mode Info', 'mocha').'</span></a>';
				echo '</li>';
			}//if
			...
			echo '<div id="mocha-opts-main">';
		
			foreach($this->sections as $k => $section){
				echo '<div id="'.$k.'_section_group'.'" class="mocha-opts-group-tab">';
				do_settings_sections($k.'_section_group');
				echo '</div>';
			}
		
		
			echo '<div id="import_export_default_section_group'.'" class="mocha-opts-group-tab">';
			echo '<h3>'.esc_html__('Import / Export Options', 'mocha').'</h3>';

			echo '<h4>'.esc_html__('Import Options', 'mocha').'</h4>';

			echo '<p><a href="javascript:void(0);" id="mocha-opts-import-code-button" class="button-secondary">Import from file</a> <a href="javascript:void(0);" id="mocha-opts-import-link-button" class="button-secondary">Import from URL</a></p>';

			echo '<div id="mocha-opts-import-code-wrapper">';

			echo '<div class="mocha-opts-section-desc">';

			echo '<p class="description" id="import-code-description">'.apply_filters('mocha-opts-import-file-description',esc_html__('Input your backup file below and hit Import to restore your sites options from a backup.', 'mocha')).'</p>';
			...
			echo '<div id="mocha-opts-import-link-wrapper">';

			echo '<div class="mocha-opts-section-desc">';

			echo '<p class="description" id="import-link-description">'.apply_filters('mocha-opts-import-link-description',esc_html__('Input the URL to another sites options set and hit Import to load the options from that site.', 'mocha')).'</p>';
			...
			echo '<p id="mocha-opts-import-action"><input type="submit" id="mocha-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.esc_attr__('Import', 'mocha').'"> <span>'.apply_filters('mocha-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'mocha')).'</span></p>';
			echo '<div id="import_divide"></div>';

			echo '<h4>'.esc_html__('Export Options', 'mocha').'</h4>';
			echo '<div class="mocha-opts-section-desc">';
			echo '<p class="description">'.apply_filters('mocha-opts-backup-description', esc_html__('Here you can copy/download your themes current option settings. Keep this safe as you can use it as a backup should anything go wrong. Or you can use it to restore your settings on this site (or any other site). You also have the handy option to copy the link to yours sites settings. Which you can then use to duplicate on another site', 'mocha')).'</p>';
			echo '</div>';

			echo '<p><a href="javascript:void(0);" id="mocha-opts-export-code-copy" class="button-secondary">Copy</a> <a href="'.add_query_arg(array('feed' => 'yaopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="mocha-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="mocha-opts-export-link" class="button-secondary">Copy Link</a></p>';
			$backup_options = $this->options;
			$backup_options['mocha-opts-backup'] = '1';
			$encoded_options = '###'.serialize($backup_options).'###';
			echo '<textarea class="large-text" id="mocha-opts-export-code" rows="8">';print_r($encoded_options);echo '</textarea>';
			echo '<input type="text" class="large-text" id="mocha-opts-export-link-value" value="'.add_query_arg(array('feed' => 'yaopts-'.$this->args['opt_name'], 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" />';

			echo '</div>';		
		
			if(true === $this->args['dev_mode']){
				echo '<div id="dev_mode_default_section_group'.'" class="mocha-opts-group-tab">';
				echo '<h3>'.esc_html__('Dev Mode Info', 'mocha').'</h3>';
				echo '<div class="mocha-opts-section-desc">';
			...
			do_action('mocha-opts-after-section-items-'.$this->args['opt_name'], $this);
		
			echo '<div class="clear"></div><!--clearfix-->';
			echo '</div>';
			echo '<div class="clear"></div><!--clearfix-->';
		
			echo '<div id="mocha-opts-footer">';
		
			if(isset($this->args['share_icons'])){
				echo '<div id="mocha-opts-share">';
				foreach($this->args['share_icons'] as $link){
					echo '<a href="'.esc_url( $link['link'] ).'" title="'.esc_attr( $link['title'] ).'" target="_mocha"><img src="'.esc_attr( $link['img'] ).'"/></a>';
				}
				echo '</div>';
			}
			...
			do_action('mocha-opts-page-after-form-'.$this->args['opt_name']);
		...
		function _errors_js(){
			...
			if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('mocha-opts-errors-'.$this->args['opt_name'])){
				$errors = get_transient('mocha-opts-errors-'.$this->args['opt_name']);
				...
				echo 'jQuery("#mocha-opts-field-errors span").html("'.count($errors).'");';
				echo 'jQuery("#mocha-opts-field-errors").show();';
					
				foreach($section_errors as $sectionkey => $section_error){
					echo 'jQuery("#'.$sectionkey.'_section_group_li_a").append("<span class=\"mocha-opts-menu-error\">'.$section_error.'</span>");';
				}
					
				foreach($errors as $error){
					echo 'jQuery("#'.$error['id'].'").addClass("mocha-opts-field-error");';
					echo 'jQuery("#'.$error['id'].'").closest("td").append("<span class=\"mocha-opts-th-error\">'.$error['msg'].'</span>");';
				}
				echo '});';
				echo '</script>';
				delete_transient('mocha-opts-errors-'.$this->args['opt_name']);
			}
		 ...
		 *
		 * @since Mocha_Options 1.0.3
		 */
		function _warnings_js(){
				
			if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('mocha-opts-warnings-'.$this->args['opt_name'])){
				$warnings = get_transient('mocha-opts-warnings-'.$this->args['opt_name']);
				...
				echo '<script type="text/javascript">';
				echo 'jQuery(document).ready(function(){';
				echo 'jQuery("#mocha-opts-field-warnings span").html("'.count($warnings).'");';
				echo 'jQuery("#mocha-opts-field-warnings").show();';
					
				foreach($section_warnings as $sectionkey => $section_warning){
					echo 'jQuery("#'.$sectionkey.'_section_group_li_a").append("<span class=\"mocha-opts-menu-warning\">'.$section_warning.'</span>");';
				}
					
				foreach($warnings as $warning){
					echo 'jQuery("#'.$warning['id'].'").addClass("mocha-opts-field-warning");';
					echo 'jQuery("#'.$warning['id'].'").closest("td").append("<span class=\"mocha-opts-th-warning\">'.$warning['msg'].'</span>");';
				}
				echo '});';
				echo '</script>';
				delete_transient('mocha-opts-warnings-'.$this->args['opt_name']);
			}
		...
		function _section_desc($section){
			...
			echo '<table class="mocha-opts-section-desc"><tr><td>';
		...
		public function prepare_field( $type = ''){
			if (!empty($type)){
				$type_class = 'Mocha_Options_'.$type;
		...
		function _field_input($field){
			...
			if ( isset($field['type']) && $this->prepare_field($field['type']) ){
				$field_class = 'Mocha_Options_'.$field['type'];;
				...
				$render->render();
				!isset($field['sub_option']) && do_action('mocha-opts-rights', $field, $this);
				echo '</td>';
