<?php
class Mocha_Options_multi_field extends Mocha_Options{	
	...
	*/
	function render(){
		...
		if(isset($this->value) && is_array($this->value)){
			foreach($this->value as $k => $value){
				foreach ($this->field['sub_fields'] as $field ) { 
					...
					$class_field = 'Mocha_Options_'.$field['type'];
		...
		echo '<a href="javascript:void(0);" class="mocha-opts-multi-field-add" rel-id="'.esc_attr( $this->field['id'] ).'-table" rel-name="'.$this->args['opt_name'].'['.$this->field['id'].'][]">'.esc_html__('Add More', 'mocha').'</a><br/>';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			'mocha-opts-field-multi-field-js', 
			MOCHA_OPTIONS_URL.'fields/multi_field/field_multi_field.js', 
			array('jquery'),
			time(),
			true
		);
