...
class Mocha_Menu_Admin {
	...
	static function options_setup() {
		self::$custom_fields = array(
			'mega_active' => array(
					'type' => 'checkbox',
					'label' => esc_html__( 'Active mega menu', 'mocha' ),
					'default' => 0					
			),
			'icon' => array(
					'type' => 'text',
					'label' => 'Icon',
					'description' => sprintf( __('Show an icon by <a href="%s">Font Awesome 4.6</a>','mocha'), esc_url( 'http://fortawesome.github.com/Font-Awesome/' ) ),
					'default' => '',
					'preview' => true,
			),
			'span' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Dropdown sub size', 'mocha' ),
					'default' => '',
					'options' => array(
						'' => 'default',
						'col-1' => '1',
			...
			'mega_full' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Active Full Mega Menu', 'mocha' ),
					'default' => 0					
			),
			'dropdown_span' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Dropdown Size', 'mocha' ),
					'default' => '',
					'options' => array(
						'column-1' => '1',
						'column-2' => '2',
			...
			'show_description_as_subtitle' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Show Description as Subtitle', 'mocha' ),
					'default' => 0					
			),
			'hide_title' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Hide Title', 'mocha' ),
					'description' =>  esc_html__( 'If icon or subtitle are using.', 'mocha' ),
					'default' => 0
			),
			'disable_link' => array(
					'type' => 'checkbox',
					'label' =>  esc_html__( 'Disable Link', 'mocha' ),
					'description' =>  esc_html__( 'Remove Link', 'mocha' ),
					'default' => 0
			),
			'advanced' => array(
					'type' => 'select',
					'label' =>  esc_html__( 'Advanced', 'mocha' ),
					'default' => '',
					'options' => array(
							'no' =>  esc_html__( 'Default', 'mocha' ),							
							'apc' =>  esc_html__( 'Append Advanced Content or Shortcode', 'mocha' ),
							'apcs' =>  esc_html__( 'Append Advanced Content with Page Select', 'mocha' )
					)
			),
			'advanced_content' => array(
					'type' => 'textarea',
					'label' =>  esc_html__( 'Advanced Content', 'mocha' ),
					'default' => '',
					'depends' => 'advanced_eq_rbd'
			),
			'page_select' => array(
					'type' => 'page_select',
					'label' =>  esc_html__( 'Page Select', 'mocha' ),
					'default' => 0,
					'description' =>  esc_html__( 'Select page to show on mega menu', 'mocha' ),					
			),
			'imgupload'		   => array(
					'type'	=> 'upload',
					'label' =>  esc_html__( 'Menu Thumbnail', 'mocha' ),
					'default' => ''
			),
		);
	}
}
add_action( 'admin_init', array('Mocha_Menu_Admin','options_setup' ) );
if ( !class_exists('Mocha_Menu') ):
...
class Mocha_Menu {
	public function __construct(){
		...
		add_action( 'admin_enqueue_scripts', array( $this,'mocha_menu_upload') );
	}
	function mocha_menu_upload(){
		wp_enqueue_script(
			'mocha-menu-upload-js',get_template_directory_uri().'/lib/admin/js/field_upload.js',
	...
	public function add_custom_nav_fields( $menu_item ){
		if ( count(Mocha_Menu_Admin::$custom_fields) ){
			foreach (Mocha_Menu_Admin::$custom_fields as $field => $field_opts){
	...
	public function update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ){
		if ( count(Mocha_Menu_Admin::$custom_fields) ){
			foreach (Mocha_Menu_Admin::$custom_fields as $field => $type){
	...
	public function edit_walker($walker, $menu_id){
		return 'Mocha_Menu_Admin_Walker';
	}
}

endif;

if ( !class_exists('Mocha_Menu_Admin_Walker') ):
 ...
 */
class Mocha_Menu_Admin_Walker extends Walker_Nav_Menu  {
	/**
	...
	function start_el(&$output, $item, $depth = 0, $args = Array(), $current_object_id = 0) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', 'mocha' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__( '%s (Pending)','mocha' ), $item->title );
		}
				...
				<dt class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'mocha'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'mocha'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr_e('Edit Menu Item', 'mocha'); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php esc_html_e( 'Edit Menu Item', 'mocha' ); ?></a>
					</span>
				</dt>
			</dl>
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Navigation Label','mocha' ); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Title Attribute','mocha' ); ?><br />
						<input type="text" id="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-attr-title" name="menu-item-attr-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->post_excerpt ); ?>" />
					</label>
				</p>
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
						<input type="checkbox" id="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>" value="_mocha" name="menu-item-target[<?php echo esc_attr( $item_id ); ?>]"<?php checked( $item->target, '_mocha' ); ?> />
						<?php esc_html_e( 'Open link in a new window/tab','mocha' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'CSS Classes (optional)','mocha' ); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)','mocha' ); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
						<?php esc_html_e( 'Description','mocha' ); ?><br />
						<textarea id="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr( $item_id ); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','mocha'); ?></span>
					</label>
				</p>
				<div class="advance-menu">
					<a href="javascript:void(0);" class="menu-advance-href" data-id="menu-advance-<?php  echo esc_attr( $item_id ) ?>"><?php esc_html_e( 'Show/Hide Advance Menu Config', 'mocha'); ?> </a>
								...
								switch( $field_opts['type'] ):
									...
									case 'select':
										...
										<?php if ( isset($field_opts['options']) && is_array($field_opts['options']) ): ?>
											...
											><?php echo ( sprintf( _n( '%s', '%s', $opt_label, 'mocha' ), $opt_label ) ); ?></option>
									...
									case 'page_select':
									?><select  id="edit-menu-item-<?php echo esc_attr( $field ); ?>-<?php echo esc_attr( $item_id ); ?>" class="edit-menu-item-<?php echo esc_attr( $field ); ?>" name="menu-item-<?php echo esc_attr( $field ); ?>[<?php echo esc_attr( $item_id ); ?>]" >
										<option value="0" selected><?php esc_html_e( 'Select page', 'mocha' ); ?></option>
										<?php $pages = get_pages(); ?>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( __('Original: %s','mocha'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php esc_html_e('Remove','mocha'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php	echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
						?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e('Cancel','mocha'); ?></a>
				</div>
...
if ( !class_exists('Mocha_Menu_Walker') ):

class Mocha_Menu_Walker extends Walker_Nav_Menu {
...
if ( !class_exists('Mocha_Mega_Menu_Walker') ):

class Mocha_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if( ($depth === 0) ){
			$class_names .= '';
			$class_names .= ( $item -> mega_active == 1 ) ? ' mocha-mega-menu' : ' mocha-menu-custom';
			$class_names .= ( $item -> mega_full == 1 ) ? ' mocha-megamenu-full' : '';
			$class_names .= ' level1';
		}		
		if( $item -> imgupload != '' ){
			$class_names .= ' mocha-menu-img';
		}
		if( $item -> icon != '' ){
			$class_names .= ' mocha-menu-icon';
		}
 ...
 */
function mocha_nav_menu_css_class( $classes, $item ) {
	...
	return array_filter($classes, 'mocha_element_empty');
}
add_filter('nav_menu_css_class', 'mocha_nav_menu_css_class', 10, 2);
 ...
 * Use Mocha_Menu_Walker() by default
*/
	...
	if (!$args['walker']) {
		if ( 'mega' == zr_options( 'menu_type' ) && $mocha_theme_locates[0] != NULL && in_array( $args['theme_location'], $mocha_theme_locates ) ){
			...
			if( !mocha_mobile_check() ) :
				wp_enqueue_script('mocha_megamenu', get_template_directory_uri() . '/js/megamenu.js', array(), null, true);
			endif;
			
		} else {
			$mocha_nav_menu_args['walker'] = new Mocha_Menu_Walker();
		}
	}

	return array_merge($args, $mocha_nav_menu_args);
}
add_filter('wp_nav_menu_args', 'mocha_nav_menu_args');
