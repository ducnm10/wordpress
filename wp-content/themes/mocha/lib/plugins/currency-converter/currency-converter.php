...
if (!class_exists('WC_Currency_Converter')) {
	...
	class WC_Currency_Converter {
		...
		public function __construct() {
			...
			// Init settings
			$this->settings = array(
				array( 'name' => esc_html__( 'Open Exchange Rate API', 'mocha' ), 'type' => 'title', 'desc' => '', 'id' => 'product_enquiry' ),
				array(
					'name' => esc_html__('App Key', 'mocha'),
					'desc' 		=> sprintf( __('(optional) If you have an <a href="%s">Open Exchange Rate API app ID</a>, enter it here.', 'mocha'), 'https://openexchangerates.org/signup' ),
		...
		function currency_conversion_js() {
			...
			wp_enqueue_script( 'mocha', get_template_directory_uri() .'/lib/plugins/currency-converter/assets/js/conversion.min.js', array( 'jquery', 'moneyjs', 'accountingjs', 'jquery-cookie' ), Null, true );
			...
			$wc_currency_converter_params = array(
				...
				'i18n_oprice'      => esc_html__( 'Original price:', 'mocha'),
				'zero_replace'     => $zero_replace
			);

			wp_localize_script( 'mocha', 'wc_currency_converter_params', apply_filters( 'wc_currency_converter_params', $wc_currency_converter_params ) );
		}
