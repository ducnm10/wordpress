<?php
class mocha_ResmenuSB{
	function __construct(){
		add_filter( 'wp_nav_menu_args' , array( $this , 'mocha_MenuRes_AdFilter' ), 100 ); 
		add_filter( 'wp_nav_menu_args' , array( $this , 'mocha_MenuRes_Filter' ), 110 );	
		add_action( 'wp_footer', array( $this  , 'mocha_MenuRes_AdScript' ), 110 );	
	}
	function mocha_MenuRes_AdScript(){
	...
	function ResNavMenu( $args ){
		$args['mocha_resmenu'] = true;		
		$select = wp_nav_menu( $args );
		return $select;
	}
	function mocha_MenuRes_Filter( $args ){
		/* Fix Menu on wp 4.7 */
		if( !isset( $args['mocha_resmenu'] ) ){
			return $args;
		}
		$args['container'] = false;
