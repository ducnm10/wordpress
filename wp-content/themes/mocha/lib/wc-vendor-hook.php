...
*/
add_action( 'wcvendors_before_dashboard', 'mocha_wrapper_before_vendor_dashboard' );
add_action( 'wcvendors_after_dashboard', 'mocha_wrapper_after_vendor_dashboard' );
function mocha_wrapper_before_vendor_dashboard(){
	echo '<div class="vendor-dashboard-wrapper">';
}

function mocha_wrapper_after_vendor_dashboard(){
	echo '</div>';
}

add_action( 'wp', 'mocha_wcvendor_hook' );
function mocha_wcvendor_hook(){
	...
	if( 'pro' !== $pro_store_header ) {
		...
		if( WCV_Vendors::is_vendor_page() ) {
			add_action( 'woocommerce_before_main_content', 'mocha_vendor_breadcrumb', 9 );
...
function mocha_vendor_breadcrumb(){
?>
...
** Add wrapper sold by
*/
add_filter( 'wcvendors_sold_by_in_loop', 'mocha_custom_soldby' );
function mocha_custom_soldby( $sold_by_label ){
