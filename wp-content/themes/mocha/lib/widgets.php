 ...
 */
function mocha_unregister_default_widgets() {
	unregister_widget('WC_Widget_Featured_Products');
	unregister_widget('WC_Widget_Best_Sellers');
	unregister_widget('WC_Widget_Top_Rated_Products');
}
add_action('widgets_init', 'mocha_unregister_default_widgets', 11);
 
/*
** Register Sidebar and Widgets
*/
function mocha_widgets_init() {
	
	// Sidebars
	global $mocha_widget_areas;
	$mocha_widget_areas = mocha_widget_setup_args();
	if ( count($mocha_widget_areas) ){
		foreach( $mocha_widget_areas as $sidebar ){
			$sidebar_params = apply_filters('mocha_sidebar_params', $sidebar);
...
class Mocha_Social_Widget extends WP_Widget{
	...
	function form($instance){ 
		...
		?>
<p>
	<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title: ', 'mocha'); ?>
...
<?php 
		...
		foreach ($socials as $j => $social){
			...
			?>
<p>
	<strong><?php echo esc_html__( 'Social', 'mocha' ). ' ' .$j+1; ?>
	</strong><br>
	<?php if (key_exists('icon', $social)){?>
	<label for="<?php echo esc_attr( $id_j_icon ); ?>"><?php esc_html_e('fa-* | title', 'mocha')?>
	...
	<?php }?>
	<label for="<?php echo esc_attr( $id_j_link ); ?>"><?php esc_html_e('Link ', 'mocha')?> </label>
...
<?php
		...
		// mocha fields for add new social network
		...
		?>
<p>
	<strong><?php esc_html_e( 'Add a new social network', 'mocha' ) ?></strong><br> <label
		for="<?php echo esc_attr( $id_i_icon ); ?>"><?php esc_html_e('Classname for Icon or Title', 'mocha'); ?>
	</label> <input class="widefat" id="<?php echo esc_attr( $id_i_icon ); ?>"
		name="<?php echo esc_attr( $name_i_icon ); ?>" type="text" value=""
		placeholder="<?php esc_attr_e( 'Enter Font Awesome icon or Title', 'mocha' ) ?>" /> <span><?php esc_html_e( 'If using as
		icon, please choose class name in Font Awesome. This is required.', 'mocha' ) ?></span>
	<label for="<?php echo esc_attr( $id_i_link ); ?>"><?php esc_html_e('Link ', 'mocha')?> </label>
...
class Mocha_Widgets{
	...
	protected $fields = array(
			'mocha_before_widget' => 'Before Widget',
			'mocha_after_widget'  => 'After Widget',
			'mocha_before_title'  => 'Before Title',
			'mocha_after_title'   => 'After Title',
			'mocha_display_conditions' => 'Display Conditions',
	...
	public function widget_display_callback( $instance, $widget, $args ){		
		...
		if ( mocha_check_wdisplay($widget_display) == false ) return false;
	...
	public function in_widget_form($widget, $r = null, $instance = array() ){
		...
		$widget_advanced = mocha_options()->getCpanelValue('widget-advanced');
		...
		<div class="advanced-opt <?php echo $adoptions_on_class; ?>">
			<div class="advanced-opt-controls" >
				<label class="button toggle" for="<?php echo esc_attr( $widget->get_field_id('adoptions') ); ?>"><?php esc_html_e( 'Advanced Options', 'mocha' ) ?>
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo esc_attr( $widget->get_field_id('widget_style') ); ?>"><?php esc_html_e( 'Widget Style', 'mocha' ) ?>
	...
	private function getDisplaySelectOptions(){
		if ( is_null($this->display_select_tpl) ){
			$this->display_select_options = $display_select_options = array(
				'all' => esc_html__('All', 'mocha'),
				'if_selected' => esc_html__('Only on selected', 'mocha'),
				'if_no_selected' => esc_html__('Except selected', 'mocha')
			);
			$html = '';
			$html .= '<p>';
			$html .= '<label for="id-display-select">'. esc_html__( 'Widget Display', 'mocha' ) .  '</label>';
	...
	private function getDisplayCheckboxUsers(){
		if ( is_null($this->display_checkbox_users_tpl) ){
			$this->display_checkbox_users = $display_checkbox_users = array(
				'login' => esc_html__('Show only for login users' , 'mocha'),
				'logout' => esc_html__('Show only for logout users' , 'mocha')
			);
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. esc_html__( 'Language Display', 'mocha' ) .  '</label>';
	...
	private function getDisplayCheckboxGeneral(){
		if ( is_null($this->display_checkbox_general_tpl) ){
			$this->display_checkbox_general = $display_checkbox_general = array(
					'home' => esc_html__('Blog posts', 'mocha'),
					'archive' => esc_html__('Archive page', 'mocha'),
					'date' => esc_html__('Archive Date', 'mocha'),
					'category' => esc_html__('Category archive', 'mocha'),
					'tag' => esc_html__('Tag archive', 'mocha'),
					'author' => esc_html__('Author archive', 'mocha'),
					'search' => esc_html__('Search page', 'mocha'),
					'single' => esc_html__('Single post', 'mocha'),
					'page' => esc_html__('Page template', 'mocha'),
					'404' => esc_html__('Page 404', 'mocha')
				);
			$html = '';
				...
				if(!isset($menu) || $menu != NULL || $menu != "")
					{
						...
						if (count($items) > 0) {
							$menu_checkbox = new Mocha_Menu_Checkbox( $menu->slug);
	...
	protected function getWidgetStyles(){
		...
		if ( is_null($this->styles) ){
			$tmp = array();
			if ( $_core_styles = $this->scandir(MOCHA_DIR.'/widgets/_styles', 'php') )
