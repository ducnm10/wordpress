 ...
 */
function mocha_embed_wrap($cache, $url, $attr = '', $post_ID = '') {
	$cache = preg_replace('/width="(.*?)?"/', 'width="100%"', $cache);
	return '<div class="entry-content-asset">' . $cache . '</div>';
}
add_filter('embed_oembed_html', 'mocha_embed_wrap', 10, 4);
add_filter('embed_googlevideo', 'mocha_embed_wrap', 10, 2);

/**
 * Add class="thumbnail" to attachment items
 */
function mocha_attachment_link_class($html) {
	$postid = get_the_ID();
	$html = str_replace('<a', '<a class="thumbnail"', $html);
	return $html;
}
add_filter('wp_get_attachment_link', 'mocha_attachment_link_class', 10, 1);
 ...
 */
function mocha_caption($output, $attr, $content) {
	if (is_feed()) {
		return $output;
	}
...
add_filter('img_caption_shortcode', 'mocha_caption', 10, 3);


/**
 * Clean up the_excerpt()
 */
function mocha_excerpt_length($length) {
	return 40;
}

function mocha_excerpt_more($more) {
	//return;
	return ' &hellip; <a href="' . get_permalink() . '">' . esc_html__('Readmore', 'mocha') . '</a>';
}
add_filter('excerpt_length', 'mocha_excerpt_length');
add_filter('excerpt_more',   'mocha_excerpt_more');

/**
 * Remove unnecessary self-closing tags
 */
function mocha_remove_self_closing_tags($input) {
  return str_replace(' />', '>', $input);
}
add_filter('get_avatar',          'mocha_remove_self_closing_tags'); // <img />
add_filter('comment_id_fields',   'mocha_remove_self_closing_tags'); // <input />
add_filter('post_thumbnail_html', 'mocha_remove_self_closing_tags'); // <img />


/**
 * Allow more tags in TinyMCE including <iframe> and <script>
 */
function mocha_change_mce_options($options) {
...
add_filter('tiny_mce_before_init', 'mocha_change_mce_options');
 ...
 */
function mocha_widget_first_last_classes($params) {
 ...
 *
 * @link http://wordpress.org/support/topic/mocha-search-sends-you-to-the-homepage#post-1772565
 * @link http://core.trac.wordpress.org/ticket/11330
 */
function mocha_request_filter($query_vars) {
...
add_filter('request', 'mocha_request_filter');



function mocha_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'mocha' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'mocha_wp_title', 10, 2 );
