...
** Build array
*/
function mocha_build_array( $case ){
	$build_arr = array();
	if( $case == 'page' ) :
		$build_arr = array( '' => esc_html__( 'Select Page', 'mocha' ) );
		$pages = get_pages(); 
		foreach( $pages as $page ) {
			$build_arr[$page->ID] = $page->post_title;
		}
	elseif( $case == 'sidebar' ) :
		$wp_registered_sidebars = mocha_widget_setup_args();
		$build_arr = array( '' => esc_html__( 'Select Sidebar', 'mocha' ) );
...
*/
function mocha_metabox_init(){
	$mocha_metabox_pages[] = array(
		'title' 	=> esc_html__( 'General', 'mocha' ),
		'fields'	=> array(
			array(
				'type'	=> 'upload',
				'title'	=> esc_html__( 'Custom Logo', 'mocha' ),
				'id'	=> 'page_logo',
				'description' => esc_html__( 'Upload custom Logo for this page', 'mocha' ),
				'std' => ''
			),
			
			array(
				'type'	=> 'select',
				'title'	=> esc_html__( 'Home Template', 'mocha' ),
				'id'	=> 'page_home_template',
				'description' => esc_html__( 'Select home template', 'mocha' ),
				'std'	 => '',
				'values' => array( '' => esc_html__( 'Default', 'mocha' ), 'home-style1' => esc_html__( 'Home Style1', 'mocha' ), 'home-style2' => esc_html__( 'Home Style2', 'mocha' ), 'home-style3' => esc_html__( 'Home Style3', 'mocha' ), 'home-style4' => esc_html__( 'Home Style4', 'mocha' ), 'home-style5' => esc_html__( 'Home Style5', 'mocha' ), 'home-style6' => esc_html__( 'Home Style6', 'mocha' ),
					)
			),
			array(
				'type'	=> 'radio_img',
				...
				'id'	=> 'scheme',
				'description' => esc_html__( 'Select one color scheme for this page', 'mocha' ),
				'std'	 => 'none',
				'values' => array( 
					'none' => '#000000',
					'default'	=> '#f61853',
	...
	$mocha_metabox_pages[] = array(
		...
		'fields'	=> array(
			array(
				'type'	=> 'text',
				'title'	=> esc_html__( 'Google Fonts', 'mocha' ),
				'id'	=> 'google_webfonts',
				'description' => esc_html__( ' Insert font style that you actually need on your webpage. Each font seperate by commas', 'mocha' ),
				'std'	 => ''	
			),
			array(
				'type'	=> 'multiselect',
				...
				'id'	=> 'webfonts_weight',
				'description' => esc_html__( 'For weight, see Google Fonts to custom for each font style.', 'mocha' ),
				'std'	 => '',
				'values' => array( 
					'100' => '100',
					'200' => '200',
			...
			array(
				...
				'id'	=> 'webfonts_assign',
				'description' => esc_html__( 'Select the place will apply the font style headers, every where or custom.', 'mocha' ),
				'std'	 => '',
				'values' => array( 
			...
			array(
				...
				'id'	=> 'webfonts_custom',
				'description' => esc_html__( 'Insert the places will be custom here, after selected custom Webfont assign.', 'mocha' ),
				'std'	 => ''	
			),		
		)
	);
	$mocha_metabox_pages[] = array(
		'title' 	=> esc_html__( 'Header', 'mocha' ),
		'fields'	=> array(
			array(
				'type'	=> 'checkbox',
				'title'	=> esc_html__( 'Hide header', 'mocha' ),
				'id'	=> 'page_header_hide',
				'description' => esc_html__( 'Choose to show or hide the header. ', 'mocha' ),
				'std' => '0'
			),
			array(
				'type'	=> 'select',
				'title'	=> esc_html__( 'Header Style Select', 'mocha' ),
	...
	$mocha_metabox_pages[] = array(
		'title' 	=> esc_html__( 'Footer', 'mocha' ),
		'fields'	=> array(
			array(
				'type'	=> 'checkbox',
				'title'	=> esc_html__( 'Hide Footer', 'mocha' ),
				'id'	=> 'page_footer_hide',
				'description' => esc_html__( 'Choose to show or hide the footer. ', 'mocha' ),
				'std'	 => '0',
			),
			array(
				'type'	=> 'select',
				'title'	=> esc_html__( 'Footer Page Select', 'mocha' ),
				'id'	=> 'page_footer_style',
				'description' => esc_html__( ' Chose to select footer page content for this page. ', 'mocha' ),
				'std'	 => '',
				'values' => mocha_build_array( 'page' )
			)
		)
	);
	
	$mocha_metabox_pages[] = array(
		'title' 	=> esc_html__( 'Sidebar', 'mocha' ),
		'fields'	=> array(
			array(
				'type'	=> 'select',
				'title'	=> esc_html__( 'Sidebar Layout', 'mocha' ),
				'id'	=> 'page_sidebar_layout',
				'description' => esc_html__( 'Choose layout sidebar for page', 'mocha' ),
				'std'	 => '',
				'values' => array( '' => esc_html__( 'Select Sidebar', 'mocha' ), 'full' => esc_html__( 'No Sidebar', 'mocha' ), 'left' => esc_html__( 'Sidebar Left', 'mocha' ), 'right' => esc_html__( 'Sidebar Right', 'mocha' ) )
			),
			array(
				'type'	=> 'select',
				'title'	=> esc_html__( 'Sidebar ', 'mocha' ),
				'id'	=> 'page_sidebar_template',
				'description' => esc_html__( ' Chose sidebar to show.', 'mocha' ),
				'std'	 => '',
				'values' => mocha_build_array( 'sidebar' )
			)		
		)
	);
	
	return $mocha_metabox_pages;
}
add_action( 'init', 'mocha_metabox_init' );

add_action( 'admin_init', 'mocha_page_init' );
function mocha_page_init(){
	add_meta_box( 'mocha_page_meta', esc_html__( 'Page Metabox', 'mocha' ), 'mocha_page_meta', array( 'page', 'post', 'product' ), 'normal', 'low' );
	add_action( 'save_post', 'mocha_page_save_meta', 10, 1 );
}	

/*
** Metabox HTML
*/
function mocha_page_meta(){
global $post;
	$mocha_metabox_pages = mocha_metabox_init();
	$except_args = array( 'General', 'Typography' );
	$current_screen =  get_current_screen()->post_type;
	wp_nonce_field( 'mocha_page_save_meta', 'mocha_metabox_plugin_nonce' );
	if( in_array( $current_screen, array( 'post', 'page', 'product' ) ) ) : 
		...
		wp_enqueue_script( 'mocha-opts-field-radio_img-js',	MOCHA_URL.'/options/fields/radio_img/field_radio_img.js',	array('jquery'), time(), true	);
	endif; 
?>
	<div class="mocha-metabox" id="mocha_metabox">
		<div class="mocha-metabox-content">
			<ul class="nav nav-tabs">
			<?php 
				$i = 0;
				foreach( $mocha_metabox_pages as $metabox ){ 
					if( ( $current_screen == 'post' || $current_screen == 'product' ) && ( in_array( $metabox['title'], $except_args ) ) ){
						continue;
					}
					$active = ( $i == 0 ) ? 'active' : '';
					echo '<li class="' . esc_attr( $active ) . '"><a href="#mocha_'. strtolower( $metabox['title'] ) .'" data-toggle="tab">' . $metabox['title'] . '</a></li>';
					$i ++;
				} 
			?>
			</ul>
			...
			?>
				<div class="tab-pane <?php echo esc_attr( $active ); ?>" id="mocha_<?php echo strtolower( $metabox['title'] ) ; ?>">
						...
						?>
							<div class="tab-inner clearfix">
								...
								<div class="flytab-content">
									<?php mocha_render_html( $meta_field['id'], $meta_field['type'], $values, $meta_field['std'] ); ?>									
								</div>
							</div>
						<?php } ?>
					<?php } ?>
...
*/
function mocha_render_html( $id, $type, $values, $std ){
	global $post;
	$meta_value = '';
	...
	switch( $type ) {
		...
		case 'radio_img' :
			...
			foreach( $values as $key => $value ) {
				$key_val = ( $key == 'none' ) ? esc_html__( 'No Select', 'mocha' ) : $key; 
				$selected = ( checked( $meta_value, $key, false ) != '' ) ? ' mocha-radio-img-selected' : '';
				$html .= '<label class="radio-label mocha-radio-img'.$selected.' mocha-radio-img-'. esc_attr( $id ) .'" for="'. esc_attr( $id ) .'_'. $i .'">';
				$html .= '<input type="radio" id="'. esc_attr( $id ) .'_'. $i .'" name="'. esc_attr( $id ) .'" value="'. esc_attr( $key ) .'" '.checked($meta_value, $key, false).'/>';
				$html .= '<div class="page-radio-color" style="background: '. esc_attr( $value ) .'" onclick="jQuery:mocha_radio_img_select(\''. esc_attr( $id ) .'_'. $i .'\', \''. esc_attr( $id ) .'\');"></div>';
		...
		?>
			<div class="upload-formfield">
				...
				<div class="metabox-thumbnail-wrapper">
					...
					<button type="button" class="upload_image_button button"><?php echo esc_html__( 'Upload/Add image', 'mocha' ) ?></button>
					<button type="button" class="remove_image_button button"><?php echo esc_html__( 'Remove image', 'mocha' ) ?></button>
				</div>
				<script type="text/javascript">
					...
					jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
						...
						file_frame = wp.media.frames.downloadable_file = wp.media({
							title: '<?php esc_html_e( "Choose an image", 'mocha' ); ?>',
							button: {
								text: '<?php esc_html_e( "Use image", 'mocha' ); ?>'
							},
							multiple: false
						});
...
function mocha_page_save_meta(){
	...
	$except_args = array( 'General', 'Typography' );
	$current_screen =  get_current_screen()->post_type;
	if ( ! isset( $_POST['mocha_metabox_plugin_nonce'] ) ) {
		return;
	}
	if ( ! wp_verify_nonce( $_POST['mocha_metabox_plugin_nonce'], 'mocha_page_save_meta' ) ) {
		return;
	}
	foreach( $mocha_metabox_pages as $key => $metabox ){ 
		if( ( $current_screen == 'post' || $current_screen == 'product' ) && ( in_array( $metabox['title'], $except_args ) ) ){
			continue;
		}
		foreach( $metabox['fields'] as $meta_field ) { 			
