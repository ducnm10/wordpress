...
function zr_activation_options_add_page() {
	...
	if ( $zr_activation_options == '' ) {
		$theme_page = add_theme_page(
				esc_html__('Theme Activation', 'mocha'),
				esc_html__('Theme Activation', 'mocha'),
				'edit_theme_options',
...
<div class="zr-activation-form">
	<div class="activation-form-inner">
		<h2>
			<?php printf( esc_html__( '%s Theme Activation', 'mocha' ), wp_get_theme() ); ?>
			<a href="<?php echo esc_url( 'http://wpthemego.com/document/how-to-get-purchase-code-for-items-from-envato/' ); ?>" target="_blank" class="zr-activation-help" title="<?php echo esc_attr__( 'Need help? Please follow this url', 'mocha' ); ?>"><?php echo esc_html__( 'Help', 'mocha' ); ?></a>
		</h2>
		<?php settings_errors(); ?>
				...
				<ul>
					<li>
						<label for="zr_purchase_code" class="clearfix">
							<input type="text" id="zr_purchase_code" placeholder="<?php echo esc_attr__( 'Enter Your Purchase Code', 'mocha' ); ?>" name="zr_purchase_code" value="<?php echo esc_attr( get_option( 'zr_purchase_code' ) ); ?>"/>
						</label>
					</li>
				</ul>
			</div>
	...
	if( zr_verify_purchase_code_result( $input ) ) {
		$new_input = $input; 
		 ...
		 add_settings_error(
			...
			esc_html__( 'Your Purchase Code are invalid, please fill correct code to action theme', 'mocha' ),
			'error'
		);
	}
	return  $new_input;
...
if( !zr_verify_purchase_code_result( get_option( 'zr_purchase_code' ) ) ) {
	function zr_admin_notice_validate_purchase_code() {
		...
		$message = esc_html__( 'Your theme is not active now, please fill your purchase code to active your theme!', 'mocha' );
