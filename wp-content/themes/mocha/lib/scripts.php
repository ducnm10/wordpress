...
function mocha_scripts() {	
	...
	$overflow_text = array(
		'more_text' => esc_html__( 'More...', 'mocha' ),
	...
	*/
	if( class_exists( 'WooCommerce' ) ) {
		...
		$mocha_dest_folder = ( function_exists( 'zr_wooswatches_construct' ) ) ? 'woocommerce' : 'woocommerce_select';
		...
		$wc_add_to_cart_variation_params = array(
			'i18n_no_matching_variations_text' => esc_attr__( 'Sorry, no products matched your selection. Please choose a different combination.', 'mocha' ),
			'i18n_make_a_selection_text'       => esc_attr__( 'Please select some product options before adding this product to your cart.', 'mocha' ),
			'i18n_unavailable_text'            => esc_attr__( 'Sorry, this product is unavailable. Please choose a different combination.', 'mocha' ),
		);
		...
		$quickview_text = array(			
			...
			'add_to_cart_variable' => esc_url( get_template_directory_uri() . '/js/'. $mocha_dest_folder .'/add-to-cart-variation.min.js' ),
	...
	*/
	...
	if( !empty( $zr_detect ) && $zr_detect->isMobile() && !$zr_detect->isTablet() ){
		...
		wp_dequeue_script( 'mocha_megamenu' );
		wp_dequeue_script( 'yith-woocompare-main' );
		wp_enqueue_script( 'mocha_mobile_js', get_template_directory_uri(). '/js/mobiles.js', array(), null, true);	
