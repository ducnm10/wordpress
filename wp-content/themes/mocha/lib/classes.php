...
abstract class Mocha_Widget extends WP_Widget{
	...
	protected function init(){
		...
		$this->base_tpl_path = apply_filters('mocha_widget_template_base', $this->base_path.'/widgets/'.$this->id_base.'/tmpl');
		$this->override_tpl_path = apply_filters('mocha_widget_override_base', get_template_directory().'/widgets/'.$this->id_base);
	}
	...
	public function widget_template_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( array_key_exists('ov', $templates) && count($templates['ov']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Override by Theme', 'mocha') .'">';
		...
		if ( array_key_exists('df', $templates) && count($templates['df']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Default Template', 'mocha') .'">';
	...
	public function category_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( (is_string($opts['multiple']) && strtolower($opts['multiple'])=='multiple') || (is_bool($opts['multiple']) && $opts['multiple']) ){
			...
			if (array_key_exists('allow_select_all', $opts) && $opts['allow_select_all']){
				...
				$allow_select_all = '<option value="0">'. esc_html_e( 'All Categories', 'mocha' ) .'</option>';
			}
		}
