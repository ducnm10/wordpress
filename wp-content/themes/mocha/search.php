...
<div class="container">
	...
	<?php
		$mocha_post_type = isset( $_GET['search_posttype'] ) ? $_GET['search_posttype'] : '';
		if( ( $mocha_post_type != '' ) &&  locate_template( 'templates/search-' . $mocha_post_type . '.php' ) ){
			get_template_part( 'templates/search', $mocha_post_type );
		}else{ 
			if( have_posts() ){
		?>
