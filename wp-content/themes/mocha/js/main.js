(function($) {
	...
	*/
	...
	if( $('body').hasClass( 'zr-wooswatches' ) ){
		$( '.zr-wooswatches .product-type-variable' ).each(function(){
			var h_swatches = $(this).find( '.zr-variation-wrapper' ).height() + 20;
	...
	*/
	...
	$('.header-style11 .header-mid .search-icon .fa-search').on('click', function(){
		$('.header-style11 .header-mid .search-cate .mocha_top .top-form.top-search').slideToggle();
	});
	...
	*/
	...
	$('.header-style12 .zr-setting').on('click', function(){
		$('.header-style12 .zr-setting .mid-header3').slideToggle("open");
	});
	...
	*/
	...
	if( w_width <= 480){
		jQuery('.mobile-layout .header-mobile-style5 .mocha_resmenu')
		.find(' > li:gt(6) ') 
		.hide()
		.end()
		.each(function(){
	...
	*/
	$('form#login_ajax').on('submit', function(e){
		...
		$.ajax({
			...
			data: { 
				'action': 'mocha_custom_login_user', //calls wp_ajax_nopriv_ajaxlogin
