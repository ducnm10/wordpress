<?php
if( !function_exists('mocha_comment') ){
	function mocha_comment($comment, $args, $depth) {
				...
				<div class="media">
					<div class="media-heading clearfix">
						...
						<div class="time pull-left">
							<?php edit_comment_link(__('(Edit)', 'mocha'), '', ''); ?>
							<time datetime="<?php echo comment_date('c'); ?>"><?php printf(__('%1$s', 'mocha'), get_comment_date(),  get_comment_time()); ?></time>
						</div>
					...
					<?php if ($comment->comment_approved == '0') : ?>
						<div class="awaiting row-fluid">
						  <i><?php esc_html_e('Your comment is awaiting moderation.', 'mocha'); ?></i>
						</div>
					<?php endif; ?>
	...
	<div id="comments">
		...
		<?php if (post_password_required()) : ?>
			<div class="alert alert-warning alert-dismissible" role="alert">
				...
				<p><?php esc_html_e('This post is password protected. Enter the password to view comments.', 'mocha'); ?></p>
			</div>
		<?php else:  ?>
		
		<div class="commentlist">
			<div class="entry-summary">
				<?php wp_list_comments(array('callback' => 'mocha_comment')); ?>
			</div>
		</div>
			...
			<nav id="comments-nav" class="pager">
				<ul class="pager">
					<?php if (get_previous_comments_link()) : ?>
						<li class="previous"><?php previous_comments_link(__('&larr; Older comments', 'mocha')); ?></li>
					<?php else: ?>
						<li class="previous disabled"><a><?php esc_html_e('&larr; Older comments', 'mocha'); ?></a></li>
					<?php endif; ?>
					<?php if (get_next_comments_link()) : ?>
						<li class="next"><?php next_comments_link(__('Newer comments &rarr;', 'mocha')); ?></li>
					<?php else: ?>
						<li class="next disabled"><a><?php esc_html_e('Newer comments &rarr;', 'mocha'); ?></a></li>
					<?php endif; ?>
				</ul>
			</nav>
