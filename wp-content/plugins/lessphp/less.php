...
function sw_less_construct(){
	...
	if (  mocha_options()->getCpanelValue('developer_mode') ){
		...
		$ya_direction = mocha_options()->getCpanelValue( 'direction' );
		...
		if ( file_exists($scheme_vars) ){
			...
			try {
				...
				// $less_variables['sidebar-width'] = mocha_options()->sidebar_collapse_width.'px';
